...
| `SERVER_PORT`                           | The port the server should use. (Only use this when running the server natively)                                                                                     | `8080`              | Any non-reserved free port (1024 to 49151)                                                   |
...
| ~~`RAWG_API_EXCLUDE_STORES`~~ | This is a list of stores that the RAWG matching algorithm will exclude. You can specify these stores as a comma-separated list with no spaces. To find the Store IDs, refer to [stores.enum.ts](https://github.com/Phalcode/gamevault-backend/blob/16a33507a190501005d39a88e2bbca3f08f95839/src/modules/providers/rawg/models/stores.enum.ts). By default, we exclude itch.io to filter out a large number of asset flips. Deprecated in favour of `RAWG_API_INCLUDED_STORES`. | `9` (itch.io)             | Any comma-separated string of store IDs |
| `RAWG_API_INCLUDED_STORES`    | This is a list of stores that the RAWG search algorithm will search games on. You can specify these stores as a comma-separated list with no spaces. To find the Store IDs, refer to [stores.enum.ts](https://github.com/Phalcode/gamevault-backend/blob/16a33507a190501005d39a88e2bbca3f08f95839/src/modules/providers/rawg/models/stores.enum.ts). By default, we exclude itch.io (ID 9) to filter out a large number of bad asset flips.                                    | `1,2,3,4,5,6,7,8,11`      | Any comma-separated string of store IDs |
